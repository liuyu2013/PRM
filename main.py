...
#params setting
tf.compat.v1.flags.DEFINE_string("train_set", "dataset/rec_train_set.sample.txt", "the file path of train set")
tf.compat.v1.flags.DEFINE_string("validation_set", "dataset/rec_validation_set.sample.txt", "the file path of validation set")
tf.compat.v1.flags.DEFINE_string("test_set", "dataset/rec_test_set.sample.txt", "the file path of test set")
tf.compat.v1.flags.DEFINE_string("log_dirctory", "./log/", "the log directory")
tf.compat.v1.flags.DEFINE_string("saved_model_name", "drr_model.h5", "the saved model name")
tf.compat.v1.flags.DEFINE_integer("model_type", 0, "drr model type, 0:drr_base 1:drr_personalized_v1 2:drr_personalized_v2")
...
tf.compat.v1.flags.DEFINE_integer("pos_embedding_mode", 1, "param used in drr_model") # 0:use fix PE  1:use learnable PE  2:unuse PE
...
def input_generator(filename, batch_size, seq_len, repeat_cnt=-1):
    ...
    dataset = tf.data.experimental.CsvDataset([filename], record_defaults=[0,"","","","",""], field_delim='|').repeat(repeat_cnt).batch(batch_size)

    for batch in dataset:
        uid_batch, ucf_batch, icf_batch, pv_batch, iv_batch, label_batch = batch
        yield get_features(uid_batch.numpy(), ucf_batch.numpy(), icf_batch.numpy(), pv_batch.numpy(), iv_batch.numpy(), batch_size, seq_len), get_label(label_batch.numpy(), batch_size, seq_len)
...
class LRSchedulerPerStep(tf.keras.callbacks.Callback):
...
class LRSchedulerPerEpoch(tf.keras.callbacks.Callback):
...
def train():
    print("trainig....")
    if not os.path.exists(FLAGS.log_dirctory):
        os.mkdir(FLAGS.log_dirctory)
        print("create log directory:{0}".format(FLAGS.log_dirctory))
    model = get_model()
    ...
    callback_list = [ tf.keras.callbacks.TensorBoard(log_dir=FLAGS.log_dirctory),
            tf.keras.callbacks.ModelCheckpoint(FLAGS.saved_model_name, verbose=1, monitor='val_loss', save_weights_only=True, save_best_only=True),
            tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=FLAGS.early_stop_patience, verbose=1),
